...
## zmmailbox location
ZMBOX=/opt/zextras/bin/zmmailbox
...
## Looping Account zextras
for account in `su - zextras -c 'zmprov -l gaa | sort'`
do
echo "Processing mailbox $account backup..."
        $ZMBOX -z -m $account getRestURL "//?fmt=${FORMAT}$query" > $ZDUMPDIR/$account.${FORMAT}
done

echo "zextras Mailbox backup has been completed successfully."
